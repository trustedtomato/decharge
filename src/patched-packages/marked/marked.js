...
(function (global, factory) {
  ...
  var Parser_1 = /*#__PURE__*/function () {
    ...
    _proto.parse = async function parse(tokens, top) {
      if (top === void 0) {
        top = true;
      }
      ...
      for (i = 0; i < l; i++) {
        ...
        switch (token.type) {
          ...
          case 'heading':
            {
              out += await this.renderer.heading(await this.parseInline(token.tokens), token.depth, unescape(await this.parseInline(token.tokens, this.textRenderer)), this.slugger);
              continue;
            }

          case 'code':
            {
              out += await this.renderer.code(token.text, token.lang, token.escaped);
              continue;
            }
          ...
          case 'table':
            {
              ...
              for (j = 0; j < l2; j++) {
                cell += await this.renderer.tablecell(await this.parseInline(token.tokens.header[j]), {
                  header: true,
                  align: token.align[j]
                });
              }
              ...
              for (j = 0; j < l2; j++) {
                ...
                for (k = 0; k < l3; k++) {
                  cell += await this.renderer.tablecell(await this.parseInline(row[k]), {
                    header: false,
                    align: token.align[k]
                  });
                }
    ...
    _proto.parseInline = async function parseInline(tokens, renderer) {
...
export { Lexer, Parser, Renderer, Slugger, TextRenderer, Tokenizer, marked as __moduleExports, defaults, getDefaults, lexer, options, parse, parseInline, parser, setOptions, use, walkTokens };
